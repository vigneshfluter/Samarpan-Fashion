...
function calculateChange() {
    ...
    const el = document.getElementById('changeToReturn');
    if(paid === 0) { el.innerText = "--"; el.style.color = 'inherit'; return; }
    el.innerText = change >= 0 ? `₹${change.toFixed(2)}` : `Short: ₹${Math.abs(change).toFixed(2)}`;
...
function processSale(type) {
    ...
    salesHistory.unshift({ 
        id: uniqueId, serial: serialNo, date: dateStr, items: cart.length, total: total, 
        customer: cName, mobile: cMobile, details: [...cart] 
    });
    saveSales();
    ...
    setTimeout(() => {
        ...
        document.getElementById('custName').value = '';
        document.getElementById('custMobile').value = '';
        document.getElementById('changeToReturn').innerText = '--';
    }, 300);
}
